training()
{
    ...
    python PSGAN.py --texture_path batik_dataset/$batik_dataset_name --output_folder ./log/$batik_dataset_name/$experiment_name --nBlocksD $4 --nBlocksG $5 >> ./log/$batik_dataset_name/$model_hyperparameters/$model_state.txt
...
model_state_1=MLP,samefakeimg,G_upsampleConv2d,instance_noise_mean=0.1,shuffle_ds=False,real_label_smoothing=1,nBlocksD=1,nBlocksG=1,nBlocksD_Dropout=0.5

#experiment 1
model_hyperparameters_2=kernel=5,zl_dim=20,zg_dim=40,zp_dim=4,ngf=64,ndf=64,batch_size=25
model_state_2=MLP,samefakeimg,G_upsampleConv2d,instance_noise_mean=0.1,shuffle_ds=False,real_label_smoothing=1,nBlocksD=1,nBlocksG=2,nBlocksD_Dropout=0.5


training 32_Kawung $model_hyperparameters_1 $model_state_1 1 1
training 32_Kawung $model_hyperparameters_2 $model_state_2 1 2
