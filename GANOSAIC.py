...
from prepare_templates import getTemplates, getImage

from config import opt, nz, criterion
from split_inference import splitW
import time
...
desc = "fc" + str(opt.fContent) + "_ngf" + str(ngf) + "_ndf" + str(ndf) + "_dep" + str(opt.nDepG) + "-" + str(opt.nDepD)
...
fixnoise = torch.FloatTensor(1, nz, targetMosaic.shape[2]//2**opt.nDepG, targetMosaic.shape[3]//2**opt.nDepG)
...
##################################
netMix = NetED(ngf, opt.nDepG, nz, nc=3, ncIn=3, bTanh=True, Ubottleneck=opt.zGL)
...
# setup optimizer
optimizerD = optim.Adam(netD.parameters(), lr=0.0002, betas=(opt.beta1, 0.999))#netD.parameters()
optimizerU = optim.Adam([param for net in Gnets for param in list(net.parameters())], lr=0.0002, betas=(opt.beta1, 0.999))
        ...
        if i % 100 == 0:
            ...
            with torch.no_grad():
                try:
                    ...
                    fixnoise2 = torch.FloatTensor(1, nz, im.shape[2] // 2 ** opt.nDepG, im.shape[3] // 2 ** opt.nDepG)
