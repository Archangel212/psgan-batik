...
##################################
netMix =NetUskip(ngf, nDepG, nz, bSkip=opt.skip_connections, nc=3, ncIn=3, bTanh=True, Ubottleneck=opt.Ubottleneck)
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        if i == 1:
            ...
            vutils.save_image(torch.cat([content,fake],2),'%s/mosaic_epoch_%03d_%s.jpg' % (opt.output_folder, epoch,desc),normalize=True)
            ...
            with torch.no_grad():
                ...
                try:
                    try:
                        files = os.listdir(opt.test_image)
                        os.makedirs(opt.output_folder+"video/")
                    except:
                        pass
                    for f in files:
                        if not epoch%VIDEO_SAVE_FREQ==0:
                            break
                        im=getImage(opt.test_image+f, bDel=True)
                        im = im.to(device)
                        ...
                        vutils.save_image(fakebig, '%s/video/%s' % (opt.output_folder,f), normalize=True)
                except Exception as e:
